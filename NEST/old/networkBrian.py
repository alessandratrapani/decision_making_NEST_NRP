...
def getting_started():
    ...
    all_spike_trains_A = results["spike_monitor_A"].spike_trains()
    spike_train_idx_list_A = numpy.sort(results["voltage_monitor_A"].record)
    
    all_spike_trains_B = results["spike_monitor_B"].spike_trains()
    spike_train_idx_list_B = numpy.sort(results["voltage_monitor_B"].record)
    ...
    ax_rate_B = None
    ax_voltage = None
    fig, (ax_raster, ax_rate_A) = plt.subplots(2, 1, sharex=True)

    plt.suptitle("Brian (c=0.70)", fontsize=20, fontweight ='bold')

    rect = Rectangle((150, -5), 350, 190, linewidth=1, edgecolor='dimgray', facecolor='whitesmoke')

    neuron_counter = 0
    for neuron_index in spike_train_idx_list_A:
        idx_spikes, ts_spikes = get_spike_train_ts_indices(all_spike_trains_A[neuron_index])
        ax_raster.plot(ts_spikes, neuron_counter * numpy.ones(ts_spikes.shape), marker=".", c="b", ms=3, lw=0)
        neuron_counter += 1

    for neuron_index in spike_train_idx_list_B:
        idx_spikes, ts_spikes = get_spike_train_ts_indices(all_spike_trains_B[neuron_index])
        ax_raster.plot(ts_spikes, neuron_counter * numpy.ones(ts_spikes.shape), marker=".", c="r", ms=3, lw=0)
        neuron_counter += 1
    ...
    tsA = results["rate_monitor_A"].t / b2.ms
    idx_rate_A = (tsA >= t_min) & (tsA <= t_max)
    smoothed_rates_A = results["rate_monitor_A"].smooth_rate(window="flat", width=20. * b2.ms)/b2.Hz
    ax_rate_A.plot(tsA[idx_rate_A], smoothed_rates_A[idx_rate_A], c="b")
    ax_rate_A.set_ylabel("A(t) [Hz]")
    ax_rate_A.set_title("Activity of pop. A", fontsize=14.5)
    ax_rate_A.set_ylim([0, 110])

    tsB = results["rate_monitor_B"].t / b2.ms
    idx_rate_B = (tsB >= t_min) & (tsB <= t_max)
    smoothed_rates_B = results["rate_monitor_B"].smooth_rate(window="flat", width=20. * b2.ms)/b2.Hz
    ax_rate_A.plot(tsB[idx_rate_B], smoothed_rates_B[idx_rate_B], c="r")
    ax_rate_A.set_ylabel("A(t) [Hz]")
